...
Public Class Cuadrillas
    ...
    'Private Sub TxtCapataz_KeyPress(sender As Object, e As KeyPressEventArgs) Handles TxtCapataz.KeyPress
    '    e.KeyChar = ControlaTecla(sender, e.KeyChar, 0, False, 5)
    ...
    'Private Sub TxtTransporte2_KeyPress(sender As Object, e As KeyPressEventArgs) Handles TxtTransporte2.KeyPress
    '    e.KeyChar = ControlaTecla(sender, e.KeyChar, 2, False, 5)
    '    If e.KeyChar = Chr(0) Then e.Handled = True
    'End Sub
    ...
    Private Sub CmdProcesarDia_Click(sender As Object, e As EventArgs) Handles CmdProcesarDia.Click
        ...
        Dim RsCapataz As cnRecordset.CnRecordset
        ...
        Sql = Trim(Sql) + " AND   ((ENTRADAS_ALBARANES.CODIGO_VARIEDAD BETWEEN '09' AND '09z') or (ENTRADAS_ALBARANES.CODIGO_VARIEDAD BETWEEN '14' AND '14z')) AND NOT( ENTRADAS_ALBARANES.OBSERVACIONES LIKE '%*KILOS*%')"
        ...
        RsCapataz = New cnRecordset.CnRecordset
        RsCapataz.Open(Sql, ObjetoGlobal.cn)
        If RsCapataz.RecordCount = 0 Then
            MsgBox("No existen este capataz")
            Return
        End If

        Sql = "SELECT * FROM OPERARIOS_COOP WHERE CUADRILLA = '" + CStr(TxtCapataz.Text) + "' and tipo_operario ='" & RsCapataz!tipo_operario & "'"
        ...
        For i = 1 To RsCampos.RecordCount
            ...
            Sql = Trim(Sql) + " AND E.CODIGO_VARIEDAD BETWEEN '09' AND '09z' AND NOT( E.OBSERVACIONES LIKE '%*KILOS*%')"
    ...
    'Private Sub TxtOperario_KeyPress(sender As Object, e As KeyPressEventArgs) Handles TxtOperario.KeyPress
    '    e.KeyChar = ControlaTecla(sender, e.KeyChar, 0, False, 5)
    '    If e.KeyChar = Chr(0) Then e.Handled = True
    'End Sub
    ...
    Private Sub CmdGrabar_Click(sender As Object, e As EventArgs) Handles CmdGrabar.Click
        ...
        SQL = "SELECT * FROM kilos_recol_var WHERE EMPRESA = '" + Trim(ObjetoGlobal.EmpresaActual) + "' AND EJERCICIO = '" + Trim(ObjetoGlobal.EjercicioActual) + "' AND CODIGO_VARIEDAD = 'F'"
        ...
        For i = 1 To RsCampos.RecordCount
            RsCampos.AbsolutePosition = i
            SQL = "SELECT * FROM ENTRADAS_RECOL WHERE EMPRESA = '" + Trim(ObjetoGlobal.EmpresaActual) + "' AND EJERCICIO = '" + Trim(ObjetoGlobal.EjercicioActual) + "' AND CODIGO_CAMPO = " + CStr(RsCampos!codigo_campo) + " AND FECHA = '" + Format(CDate(DTFechaEntrada.Value), "dd/MM/yyyy") + "' AND CODIGO_VARIEDAD = 'F' AND CAPATAZ = " + CStr(RsCampos!Capataz) + " ORDER BY EMPRESA,EJERCICIO,SERIE_ALBARAN,NUMERO_ALBARAN"
            Rs.Open(SQL, ObjetoGlobal.cn)
            If Rs.RecordCount > 0 Then
