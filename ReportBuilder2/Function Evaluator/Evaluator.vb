Public Class Evaluator
   ...
   Private Class tokenizer
      ...
      Public Sub ParseString(ByVal InQuote As Boolean)
         ...
         Loop
         If InQuote Then
                RaiseError("Cadena incompleta, falta " & OriginalChar & "; String started")
            End If
      End Sub
               ...
               Case eTokenType.open_parenthesis
                  ...
                  ValueLeft = ParseExpr(0, ePriority.none)
                        If tokenizer.type = eTokenType.close_parenthesis Then
                            ...
                            Exit Do
                        Else
                            tokenizer.RaiseUnexpectedToken("No encuentro el paréntesis final")
                        End If
               Case Else
                  Exit Do
            End Select
            ...
            Select Case tt
               ...
               Case eTokenType.operator_percent
                  If priority < ePriority.percent Then
                     ...
                     Else
                        ...
                        End If
                                tokenizer.RaiseError("No puedo usar el operador + or - on a " & ValueLeftString & " y " & valueRight.GetType.Name & vbCrLf _
                           & "Puede utilizar % solamente con números. Por ejemplo: 150 + 20.5% ")
                            End If
                  Else
                     Exit Do
                  End If
               Case eTokenType.operator_or
                  If priority < ePriority.or Then
                     ...
                     Else
                                tokenizer.RaiseError("No puedo usar el operador + or - on a  " & ValueLeft.GetType.Name & " y " & valueRight.GetType.Name & vbCrLf _
                           & "sólo puedes utilizar OR con valores booleanos")
                            End If
                  Else
                     Exit Do
                  End If
               Case eTokenType.operator_and
                  If priority < ePriority.and Then
                     ...
                     valueRight = ParseExpr(ValueLeft, ePriority.and)
                            If TypeOf ValueLeft Is Boolean And TypeOf valueRight Is Boolean Then
                                ValueLeft = CBool(ValueLeft) And CBool(valueRight)
                            Else
                                tokenizer.RaiseWrongOperator(tt, ValueLeft, valueRight,
                           "Puedes usar [op] solamente con valores booleanos")
                            End If
                        Else
                     Exit Do
                  End If
   ...
   Public Function EvalDouble(ByRef formula As String) As Double
      ...
      If TypeOf res Is Double Then
         Return CDbl(res)
      Else
            Throw New parserexception("El resultado no es un número : " & res.ToString)
        End If
   End Function
