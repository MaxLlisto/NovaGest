...
Public Class FrmNEntradasNuevo01T
    ...
    Dim ReferenciaCampoTerceros() As String ' CodigoSocio (10) + Variedad(10) + CodigoCampo(10)
    Dim DatosCampoTerceros() As String 'Situación(10) + Descripcion(60) + Hanegadas(##.####)(7)+ DESCRIPCION_VARIEDAD (30)
    Dim CuantosCamposTerceros As Long
    Dim ReferenciaSocioTerceros() As String ' Apellidos + Nombre(80) + CodigoSocio(10)


    Private Sub FrmNEntradasNuevo01T_Load(sender As Object, e As EventArgs) Handles Me.Load
        Dim i As Integer
    ...
    Private Sub TxtCapataz_Validating(sender As Object, e As CancelEventArgs) Handles TxtCapataz.Validating
        ...
        Else
            ...
            If Rs.RecordCount = 0 Then
                ...
                lblCapataz.Text = ""
            Else
                lblCapataz.Text = "" & Trim(Rs!Nombre)
            End If
            Rs.Close()
        End If
    ...
    Private Sub TxtCodigosocio_Validating(sender As Object, e As CancelEventArgs) Handles TxtCodigosocio.Validating
        ...
        If FlagCambioEnCodigo Then
            If Trim(TxtCodigosocio.Text) = "" Then
                Txtnombre.Text = ""
                MsgBox("El código de socio no puede permanecer vacio")
                e.Cancel = True
            Else
                Rs.Open("SELECT * FROM proveedores_coop WHERE codigo_proveedor = " + CStr(TxtCodigosocio.Text), ObjetoGlobal.cn)
                If Rs.RecordCount = 0 Then
                    Txtnombre.Text = ""
                    MsgBox("El código de socio indicado es incorrecto")
                    e.Cancel = True
                Else
                    Txtnombre.Text = Trim("" & Rs!razon_social)
                End If
                Rs.Close()
            End If
        End If
    ...
    Private Function ControlAgendayplazo(CodCampo, CodVariedad)
        ...
        If Strings.Left(CodVariedad, 2) = "01" Or
           ...
           (Strings.Left(CodVariedad, 3) >= "141" And Strings.Left(CodVariedad, 3) <= "144") Or
            ...
            If Trim(CodCampo) > "" And Trim(CodVariedad) > "" Then
                RsCult.Open("SELECT * FROM CAMPOS_TERCEROS WHERE " &
                ...
                If RsCult!agenda <> "S" Then
                    MsgBox("No tiene agenda", MsgBoxStyle.Critical)
                    Return False
                End If
                RsCult.Close()
            End If
        ...
        RsOr.Open("SELECT empresa FROM Ordenes_rec_terc WHERE empresa='" &
        ...
        If RsOr.RecordCount = 0 Then
            MsgBox("No tiene orden de recolección", MsgBoxStyle.Critical)
            Comprobacion = False
            Return False
        End If
        RsOr.Close()
    ...
    Private Sub CmdRecalcularBultos_Click(sender As Object, e As EventArgs)
    ...
    Private Function VerTramitesPrevios()
        ...
        '            SQL = "SELECT PARTES_SERVICIOS.SITUACION,PARTES_SERVICIOS.FECHA_FIN_PARTE,PARTES_TRATAMIENTOS.*,TARIFAS_ARTICULO.PLAZO_SEGURIDAD,SERVICIOS.* FROM PARTES_TRATAMIENTOS strings.Left JOIN PARTES_SERVICIOS ON PARTES_SERVICIOS.EMPRESA = PARTES_TRATAMIENTOS.EMPRESA AND PARTES_SERVICIOS.CODIGO_PARTE = PARTES_TRATAMIENTOS.CODIGO_PARTE"
        '            SQL = Trim(SQL) + " strings.Left JOIN TARIFAS_ARTICULO ON PARTES_TRATAMIENTOS.EMPRESA = TARIFAS_ARTICULO.EMPRESA AND PARTES_TRATAMIENTOS.CODIGO_ARTICULO = TARIFAS_ARTICULO.CODIGO_ARTICULO"
        '            SQL = Trim(SQL) + " strings.Left JOIN SERVICIOS ON PARTES_SERVICIOS.EMPRESA = SERVICIOS.EMPRESA AND PARTES_SERVICIOS.CODIGO_SERVICIO = SERVICIOS.CODIGO_SERVICIO"
    ...
    Private Sub PoblarListaSocios(Cadena As String)
        ...
        Do While (P1 <= P2 And Primero = False)
            ...
            LstProceso.Items.Add(UCase(ReferenciaSocioTerceros(PM)))
            Aceptable = False : Primero = False
            If Comparar(Strings.Left(UCase(ReferenciaSocioTerceros(PM)), Len(Cadena)), UCase(Cadena)) = 0 Then
                Aceptable = True
                If PM = 1 Then
                    Primero = True
                Else
                    If Comparar(Strings.Left(UCase(ReferenciaSocioTerceros(PM - 1)), Len(Cadena)), UCase(Cadena)) <> 0 Then Primero = True
                End If
            End If
        ...
        LstProceso.Items.Add(ReferenciaSocioTerceros(PM))

        If PM > 1 Then LstProceso.Items.Add(ReferenciaSocioTerceros(PM - 1))

        If Primero = True Then
            For j = PM To CuantosSocios
                If Comparar(Strings.Left(UCase(ReferenciaSocioTerceros(j)), Len(Cadena)), UCase(Cadena)) = 0 Then
                    Item = LstSocios.Items.Add(Trim(Strings.Left(ReferenciaSocioTerceros(j), 40)) + " " + Trim(Strings.Mid(ReferenciaSocioTerceros(j), 41, 40)))
                    Item.SubItems.Add(CStr(CLng(Strings.Mid(ReferenciaSocioTerceros(j), 81))))
                Else
                    Exit For
                End If
            Next
            LstSocios.Items(1).Selected = True
            PoblarListaCampos(CLng(Strings.Mid(ReferenciaSocioTerceros(PM), 81)), Trim(Strings.Left(ReferenciaSocioTerceros(PM), 80)), "")
        End If
    ...
    Private Sub PoblarListaCampos(Socio As Long, Nombre As String, Variedad As String)
        ...
        Do While (P1 <= P2 And Primero = False)
            PM = Fix((P1 + P2) / 2)
            LstProceso.Items.Add(CStr(P1) + " " + CStr(P2) + " " + CStr(PM))
            LstProceso.Items.Add(UCase(ReferenciaCampoTerceros(PM)))
            Aceptable = False : Primero = False
            If Comparar(Strings.Left(UCase(ReferenciaCampoTerceros(PM)), 10), UCase(Cadena)) = 0 Then
                Aceptable = True
                If PM = 1 Then
                    Primero = True
                Else
                    If Comparar(Strings.Left(UCase(ReferenciaCampoTerceros(PM - 1)), 10), UCase(Cadena)) <> 0 Then Primero = True
                End If
            End If
            If Aceptable = True And Primero = True Then
                P1 = PM : P2 = PM
        ...
        LstProceso.Items.Add(CStr(PM) + " PRIMERO = " + CStr(Primero))
        LstProceso.Items.Add(ReferenciaCampoTerceros(PM))
        If PM > 1 Then
            LstProceso.Items.Add(ReferenciaCampoTerceros(PM - 1))
        End If

        If Primero = True Then
            For j = PM To CuantosCamposTerceros
                If Comparar(Strings.Left(UCase(ReferenciaCampoTerceros(j)), 10), UCase(Cadena)) = 0 Then
                    Item = LstCampos.Items.Add(CStr(CLng(Mid(ReferenciaCampoTerceros(j), 21))))
                    Item.SubItems.Add(Mid(ReferenciaCampoTerceros(j), 11, 10) + " " + Trim(Mid(DatosCampoTerceros(j), 78)))
                    Item.SubItems.Add(Trim(Mid(DatosCampoTerceros(j), 11, 60)))
                    Item.SubItems.Add(CStr(CDbl(Mid(DatosCampoTerceros(j), 71, 7))))
                    Item.SubItems.Add(Trim(Mid(ReferenciaCampoTerceros(j), 11, 10)))
                Else
                    Exit For
                End If
            Next
    ...
    Private Sub TxtTipoenvase_Validating(sender As Object, e As CancelEventArgs) Handles TxtTipoenvase.Validating
        Dim Rs As cnRecordset.CnRecordset = New cnRecordset.CnRecordset
        Dim i As Integer
    ...
    Private Sub TxtTotalbultos_Validating(sender As Object, e As CancelEventArgs)
    ...
    Private Function Comprobacion() As Boolean
        ...
        If RsOperario.RecordCount = 0 Then
            MsgBox("Capataz inexistente en la tabla de personal terceros")
            Return False
        End If
        RsOperario.Close()
        ...
        If RsOperario.RecordCount = 0 Then
            MsgBox("Transportista inexistente en la tabla de personal terceros")
            Return False
        End If
        RsOperario.Close()
        ...
        If SocioSeleccionado <= 0 Then
            MsgBox("No se ha indicado codigo de proveedor.")
            Return False
        End If
        ...
        If RsCampo.RecordCount = 0 Then
            MsgBox("Campo tercero inexistente")
            Return False
        End If
    ...
    Private Function GrabarEntrada() As Boolean
        ...
        Try
            ...
            RsEntradas!campo_terceros = CampoSeleccionado
            RsEntradas!codigo_proveedor = SocioSeleccionado
            TipoDeAlbaran = "T"
    ...
    Private Sub FrmNEntradasNuevo01T_KeyDown(sender As Object, e As KeyEventArgs) Handles Me.KeyDown
        Select Case e.KeyCode
            ...
            Case Keys.F5 ' Capataz
                Dim oFrm As FrmNEntradasNuevo02T = New FrmNEntradasNuevo02T
                oFrm.pform = Me
