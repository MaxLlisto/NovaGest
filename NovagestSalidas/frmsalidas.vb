...
Inherits LibreriaModeloMantenimiento.ModeloMantenimiento
    ...
    Public Function ObtenerNumeroLineaSalida(empresa As String, Ejercicio As String, Serie As String, TipoDocumento As String, NumeroDocumento As String) As Long
        Dim RsLineas As cnRecordset.CnRecordset = New cnRecordset.CnRecordset
        Dim Contador As Long = 0

        RsLineas.Open("SELECT EMPRESA,EJERCICIO,SERIE,TIPO_DOCUMENTO,NUMERO_DOCUMENTO,NUMERO_LINEA FROM LINEAS_SALIDA WHERE EMPRESA = '" + Trim(empresa) + "'  AND EJERCICIO = '" + Trim(Ejercicio) + "' AND SERIE= '" + Trim(Serie) + "' AND TIPO_DOCUMENTO = '" + Trim(TipoDocumento) + "' AND NUMERO_DOCUMENTO = '" + Trim(NumeroDocumento) + "' order by 1,2,3,4,5,6 desc", ObjetoGlobal.cn)
        If RsLineas.RecordCount > 0 Then
            Contador = RsLineas!numero_linea
        End If
        RsLineas.Close()
        Return (Contador + 1)
    ...
    Public Function ObtenerNumeroSalida(empresa As String, Ejercicio As String, Serie As String, TipoDocumento As String, UtilizaPendientes As Boolean) As Long
        Dim RsPendientes As cnRecordset.CnRecordset
        Dim RsContadores As cnRecordset.CnRecordset
        Dim RsCabeceras As cnRecordset.CnRecordset
        Dim RsLineas As cnRecordset.CnRecordset
        ...
        Try
            ...
            If UtilizaPendientes = True Then
                RsPendientes.Open("SELECT * FROM CONTADORES_PEND WHERE EMPRESA = '" + Trim(empresa) + "' AND EJERCICIO = 'T' AND NOMBRE = 'ultima_salida' AND SERIE = '" + Trim(Serie) + "' ORDER BY 1,2,3,4,5", ObjetoGlobal.cn, True,,,,,, trans)
                ...
                RsPendientes.Close()
            End If
            RsContadores.Open("SELECT * FROM CONTADORES WHERE EMPRESA = '" + Trim(empresa) + "' AND EJERCICIO = 'T' AND NOMBRE = 'ultima_salida'  AND SERIE = '" + Trim(Serie) + "'", ObjetoGlobal.cn, True,,,,,, trans)
            If RsContadores.RecordCount <= 0 Then
                ...
                RsContadores!Nombre = "ultima_salida"
            ...
            Do While FlagExiste = True
                FlagExiste = False
                RsCabeceras.Open("SELECT EMPRESA FROM CABECERAS_SALIDA WHERE EMPRESA = '" + Trim(empresa) + "' AND SERIE= '" + Trim(Serie) + "' AND NUMERO_DOCUMENTO = '" + Format(Contador, "00000000") + "'", ObjetoGlobal.cn, True,,,,,, trans)
                If RsCabeceras.RecordCount > 0 Then
                    Contador = Contador + 1
                    FlagExiste = True
                    FlagCambiado = True
                End If
                ...
                If FlagExiste = False Then
                    RsLineas.Open("SELECT EMPRESA FROM LINEAS_SALIDA WHERE EMPRESA = '" + Trim(empresa) + "' AND SERIE= '" + Trim(Serie) + "' AND NUMERO_DOCUMENTO = '" + Format(Contador, "00000000") + "'", ObjetoGlobal.cn, True,,,,,, trans)
                    If RsLineas.RecordCount > 0 Then
                        Contador = Contador + 1
                        FlagExiste = True
                        FlagCambiado = True
        ...
        Catch ex As Exception
            trans.Rollback()
            ObjetoGlobal.cn.Close()
            MsgBox("No se puede obtener el contador de albaranes salida." + vbCrLf + "Se ha producido el siguiente mensaje:" + vbCrLf + Err.Description)
            Return -1
    ...
    Protected Overrides Function HOOK_Antes_de_Crear_Ficha(CT As CnTabla.CnTabla) As Boolean
        ...
        Dim NumDoC As Long
        Dim NumLin As Long
        Dim CE As CnEdicion.CnEdicion = New CnEdicion.CnEdicion
        Dim Campos() As String
        Dim CamposTabla() As String
        Dim Valores() As String
        Dim Bultos As Long
        Dim Kilos As Long
        Dim BultosHay As Long
        Dim KilosHay As Long
        Dim Posicion As Long

        If CT.Tabla = "cabeceras_salida" Then
            NumDoC = ObtenerNumeroSalida(CT.ValorCampo("empresa"), CT.ValorCampo("ejercicio"), CT.ValorCampo("serie"), CT.ValorCampo("numero_documento"), True)
            If NumDoC = -1 Then
                MsgBox("Se ha producido un error al obtener número de salida")
            Else
                ReDim Campos(0), Valores(0), CamposTabla(0)
                Campos(0) = "numero_documento"
                Valores(0) = Format(NumDoC, "00000000")
                HOOK_AsignarValores(CT, Campos, Valores)
            End If

            ReDim Campos(0), Valores(0), CamposTabla(0)
            Campos(0) = "fecha_documento"
            Valores(0) = CStr(Format(DateTime.Now, "dd/MM/yyyy"))
            HOOK_AsignarValores(CT, Campos, Valores)
            CnEdicion038.TxtDatos.Focus()

        ElseIf CT.Tabla = "lineas_salida" Then

            If (Not CnEdicion044.TxtDatos Is Nothing) Then CnEdicion044.HayValorFijoCreacion = False
            NumLin = ObtenerNumeroLineaSalida(CT.ValorCampo("empresa"), CT.ValorCampo("ejercicio"), CT.ValorCampo("serie"), CT.ValorCampo("tipo_documento"), CT.ValorCampo("numero_documento"))
            If CnEdicion044.TxtDatos Is Nothing Then
                ReDim Campos(0), Valores(0), CamposTabla(0)
                Campos(0) = "numero_linea"
            ...
            Else
                ...
                Valores(0) = CStr(NumLin)
                HOOK_AsignarValores(CT, Campos, Valores)
                CnEdicion044.ValorFijoCreacion = CnEdicion044.TxtDatos.Text
                CnEdicion044.HayValorFijoCreacion = True
            End If
            ReDim Campos(0), Valores(0), CamposTabla(0)
            Campos(0) = "valoracion"
            Valores(0) = "N"
            Rs.Open("Select * From clientes_coop WHERE codigo_cliente = " + CT.ValorCampo("codigo_cliente"), ObjetoGlobal.cn)
            If Not IsDBNull(Rs!tipo_comercial) Then
                If (Trim(Rs!tipo_comercial)) = "ANE" Then
                    Valores(0) = "A"
                End If
            End If
            HOOK_AsignarValores(CT, Campos, Valores)

        ElseIf CT.Tabla = "calibres_salida" Then
            Bultos = Rs2!Bultos
            Kilos = Rs2!neto
            Rs2 = Nothing
            BultosHay = 0 : KilosHay = 0
            If Rs.RecordCount > 0 Then
                Posicion = Rs.AbsolutePosition
                Rs.MoveFirst()
                While Rs.EOF = False
                    KilosHay = KilosHay + Rs!Kilos
                    BultosHay = BultosHay + Rs!Bultos
                    Rs.MoveNext()
                End While
                Rs.AbsolutePosition = Posicion
            End If
            If BultosHay < Bultos And KilosHay < Kilos Then
                ReDim Campos(1), Valores(1), CamposTabla(1)
                Campos(0) = "bultos"
                Valores(0) = Bultos - BultosHay
                Campos(1) = "kilos"
                Valores(1) = Kilos - KilosHay
                HOOK_AsignarValores(CT, Campos, Valores)
            End If
            'Set CE = pform.ControlTabla(QueControl).DevolverCedicion("codigo_calibre")
