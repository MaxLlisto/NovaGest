...
Inherits LibreriaModeloMantenimiento.ModeloMantenimiento
    ...
    Public Sub VolcarCalidadesVariedad()
        Dim RsCalidades As cnRecordset.CnRecordset = New cnRecordset.CnRecordset
        Dim RsCalidadesVariedad As cnRecordset.CnRecordset = New cnRecordset.CnRecordset
        Dim RsCalidadesCoop As cnRecordset.CnRecordset = New cnRecordset.CnRecordset
        Dim rsPrecios As cnRecordset.CnRecordset = New cnRecordset.CnRecordset
        Dim i As Integer

        Try
            RsCalidadesVariedad.Open("SELECT * FROM calidades_var_ej" &
            " WHERE EMPRESA='" & Trim(CnTabla01.ValorCampo("empresa")) & "' AND ejercicio = '" & Trim(CnTabla01.ValorCampo("ejercicio")) & "' AND CODIGO_VARIEDAD='" & Trim(CnTabla01.ValorCampo("codigo_variedad")) &
            "' ORDER BY CODIGO_CALIDAD", ObjetoGlobal.cn)

            If RsCalidadesVariedad.RecordCount > 0 Then
                RsCalidades = RsCalidadesVariedad
            Else
                RsCalidadesVariedad.Close()
                RsCalidadesCoop.Open("SELECT * FROM CALIDADES_COOP  WHERE EMPRESA='" & Trim(CnTabla01.ValorCampo("empresa")) & "'" &
                " ORDER BY CODIGO_CALIDAD", ObjetoGlobal.cn)
                If RsCalidadesCoop.RecordCount > 0 Then
                    RsCalidades = RsCalidadesCoop
                Else
                    Return
                End If
            End If

            rsPrecios.Open("SELECT * FROM PRECIOS_LIQUID WHERE empresa = '" & Trim(CnTabla01.ValorCampo("empresa")) & "' AND ejercicio = '" & Trim(CnTabla01.ValorCampo("ejercicio")) & "' AND CODIGO_VARIEDAD='" & Trim(CnTabla01.ValorCampo("codigo_variedad")) & "' AND CODIGO_PERIODO= '" & Trim(CnTabla01.ValorCampo("codigo_periodo")) & "'", ObjetoGlobal.cn, True)
            If rsPrecios.RecordCount = 0 Then
                For i = 1 To RsCalidades.RecordCount
                    'Actualizar la tabla "precios_liquid"
                    rsPrecios.AddNew
                    rsPrecios!empresa = Trim(CnTabla01.ValorCampo("empresa"))
                    rsPrecios!ejercicio = Trim(CnTabla01.ValorCampo("ejercicio"))
                    rsPrecios!codigo_variedad = Trim(CnTabla01.ValorCampo("codigo_variedad"))
                    rsPrecios!codigo_periodo = Trim(CnTabla01.ValorCampo("codigo_periodo"))
                    rsPrecios!codigo_calidad = Trim(RsCalidades!codigo_calidad)
                    rsPrecios!precio_anticipo = 0
                    rsPrecios!precio_liquidacion = 0
                    rsPrecios!usucreacion = ObjetoGlobal.UsuarioActual
                    rsPrecios!fechacreacion = Today
                    rsPrecios!horacreacion = Format(TimeOfDay, "HH:mm:ss")
                    rsPrecios.Update
                    RsCalidades.MoveNext()
                Next
                'CnTabla02.
            Else
                Return
        ...
        Catch ex As Exception
            MsgBox("Se han producido errores generando la tabla de precios por calidad")
        End Try
