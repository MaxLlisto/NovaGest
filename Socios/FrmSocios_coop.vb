...
Public Class FrmSocios_coop
Inherits LibreriaModeloMantenimiento.ModeloMantenimiento

    Private Sub CmdImprimeFicha_Click(sender As Object, e As EventArgs) Handles CmdImprimeFicha.Click
        Dim oFormFicha As FrmEdicionFicha = New FrmEdicionFicha

        If CnTabla01.CuantosRegistros = 0 Then
            MsgBox("Debes seleccionar un socio primero")
            Return
        End If

        oFormFicha.ObjetoGlobal = Me.ObjetoGlobal
        oFormFicha.DelSocio(CnTabla01.ValorCampo("codigo_socio"), Trim(Trim(CnTabla01.ValorCampo("nombre_socio")) + " " + CnTabla01.ValorCampo("apellidos_socio")))
        oFormFicha.NumeroCopias(2)
        oFormFicha.TipoFicha("F")
        oFormFicha.ImprimirAhora = True
        If oFormFicha.ShowDialog Then
            'oFormFicha.ImprimeFicha()
            'oFormFicha.Close()
        End If

        '    Select Case nInd
        '        Case 0 : oFormFicha.TipoFicha "F"
        '    Case 1 : oFormFicha.TipoFicha "S"
        '    Case 2 : oFormFicha.TipoFicha "D"
        'End Select
        '    oFormFicha.RangoSocios Rs!codigo_socio
        'oFormFicha.SoloCitricos False
        ''oFormFicha.CamposDocumentados
        '    oFormFicha.CamposBaja False


        '        If SituacionFormulario = Inactivo Then
        '    Set Rs = XCTabla(0).Rs(1) 'Nuevo01032004
        '    If Rs.RecordCount = 0 Then
        '                ManejoDeMensajes "Debe seleccionar previamente socio.", "FrmSociosCoop", "CmdImprimeFicha_Click", False
        '        Exit Sub
        '            End If
        '    Set oFormFicha = CreateObject("css228cp.ClsEdicionContratoN9").Formulario
        '    Set oFormFicha.ObjetoGlobal = Me.ObjetoGlobal
        '    Set oFormFicha.MiWord = MiWord
        '    oFormFicha.DelSocio Rs!codigo_socio, Trim(Trim(Rs!nombre_socio) + " " + Rs!apellidos_socio)
        '    oFormFicha.TxtCopias.Text = "2"
        '            'oFormFicha.SoloCitricos False
        '            'oFormFicha.CamposDocumentados
        '            'oFormFicha.CamposBaja False
        '            oFormFicha.Show
        '            oFormFicha.DesHabilitar
        '            oFormFicha.ImprimirFicha
        '            oFormFicha.Form_Unload 1
        '    Set oFormFicha = Nothing
        'End If
    End Sub

    Private Sub CmdImprimeContrato_Click(sender As Object, e As EventArgs) Handles CmdImprimeContrato.Click
        Dim oFormFicha As FrmEdicionContrato = New FrmEdicionContrato
    ...
    Private Sub CmdImprimeseguros_Click(sender As Object, e As EventArgs) Handles CmdImprimeseguros.Click
        Dim oFormFicha As FrmEdicionFicha = New FrmEdicionFicha
    ...
    Private Sub CmdImprimeEfectivos_Click(sender As Object, e As EventArgs) Handles CmdImprimeEfectivos.Click
        Dim oFormFicha As FrmEdicionFicha = New FrmEdicionFicha
    ...
    Protected Overrides Function HOOK_Antes_de_Crear_Ficha(CT As CnTabla.CnTabla) As Boolean
        Dim Campos(1), Valores(1)

        If CT.Tabla.ToLower = "campos" Then
            Dim Rs As cnRecordset.CnRecordset = New cnRecordset.CnRecordset
            Rs.Open("SELECT dbo.fn_primer_hueco_campos('" + Trim(ObjetoGlobal.EmpresaActual) + "') as hueco", ObjetoGlobal.cn)
            ReDim Campos(1), Valores(1)
            Campos(0) = "empresa"
            Valores(0) = Trim(ObjetoGlobal.EmpresaActual)
            Campos(1) = "codigo_campo"
            Valores(1) = Rs!hueco
            Rs.Close()
            HOOK_AsignarValores(CT, Campos, Valores)
        ElseIf CT.Tabla.ToLower = "cultivos_sig" Then
            If (Not CnEdicion220 Is Nothing) Then CnEdicion220.HayValorFijoCreacion = False
            Dim NumLin As Integer
            NumLin = ObtenerContadorCultivosSIG(CnTabla05.ValorCampo("empresa"), CnTabla05.ValorCampo("ejercicio"), CnTabla05.ValorCampo("codigo_campo"), CnTabla05.ValorCampo("codigo_variedad"))
            If CnTabla05.ValorCampo("contador") Is Nothing Then
    ...
    ' HOOK_AsignarValores(CT, Campos, Valores, GE, HH)
    'End If
    Function ObtenerContadorCultivosSIG(empresa As String, CodigoCampo As Long, Ejercicio As String, Variedad As String) As Long
        Dim RsLineas As cnRecordset.CnRecordset = New cnRecordset.CnRecordset
        Dim Ret As Integer

        RsLineas.Open("SELECT MAX(CONTADOR) AS CUAL FROM CULTIVOS_SIG WHERE EMPRESA = '" & Trim(empresa) & "' AND CODIGO_CAMPO = " & CStr(CodigoCampo) & " AND EJERCICIO = '" + Trim(Ejercicio) & "' AND CODIGO_VARIEDAD = '" & Trim(Variedad) & "'", ObjetoGlobal.cn)
        If RsLineas.RecordCount <= 0 Or IsDBNull(RsLineas!cual) Then
            RsLineas.Close()
            Return 1
        Else
            Ret = (RsLineas!cual + 1)
