...
Inherits LibreriaModeloMantenimiento.ModeloMantenimiento
    ...
    '    If CT.Estado = CnTabla.CnTabla.EstadoCnTabla.Creando And Trim(CT.Tabla) = "personal_marcajes" Then
    '        ReDim Preserve Campos(1), Valores(1)
    '        Campos(0) = "actualizado" : Valores(0) = "N"
    ...
    Public Overrides Function HOOK_Despues_de_modificar_campo_en_creacion_grid(CT As CnTabla.CnTabla, CE As CnEdicion.CnEdicion, Valor As String, GE As DataGridView, HH As Hashtable) As Boolean
        ...
        If CT.Estado = CnTabla.CnTabla.EstadoCnTabla.Creando And Trim(CE.Campo) = "hora_marcaje" Then
            ReDim Preserve Campos(0), Valores(0)
            Campos(0) = "hora_marcaje" : Valores(0) = ""

            cHora = Valor
            If VerificarHora(cHora) Then
                Valores(0) = cHora
            Else
                Valores(0) = ""
                HOOK_Despues_de_modificar_campo_en_creacion_grid = True
            End If
            HOOK_AsignarValores(CT, Campos, Valores, GE, HH)
            If CT.Estado = CnTabla.CnTabla.EstadoCnTabla.Creando And Trim(CE.Campo) = "tipo_marcaje" Then
            End If

        ElseIf CT.Estado = CnTabla.CnTabla.EstadoCnTabla.Creando And Trim(CE.Campo) = "tipo_marcaje" Then
            ReDim Campos(0), Valores(0)

            If UCase(Trim(Valor)) = "E" Or UCase(Trim(Valor)) = "S" Then
                Campos(0) = Trim(Trim(CE.Campo))
                Valores(0) = UCase(Trim(Valor))
                HOOK_AsignarValores(CT, Campos, Valores, GE, HH)
            Else
                Campos(0) = Trim(CE.Campo)
                Valores(0) = ""
                HOOK_AsignarValores(CT, Campos, Valores, GE, HH)
                HOOK_Despues_de_modificar_campo_en_creacion_grid = True
            End If
        ElseIf CT.Estado = CnTabla.CnTabla.EstadoCnTabla.Creando And Trim(CE.Campo) = "actualizado" Then
            ReDim Campos(0), Valores(0)

            If UCase(Trim(Valor)) = "S" Or UCase(Trim(Valor)) = "N" Then
                Campos(0) = Trim(Trim(CE.Campo))
                Valores(0) = UCase(Trim(Valor))
                HOOK_AsignarValores(CT, Campos, Valores, GE, HH)
            Else
    ...
    ''    If CT.Estado = CnTabla.CnTabla.EstadoCnTabla.AceptandoCreacion And Trim(CT.Tabla) = "personal_marcajes" Then
    ''        If String.IsNullOrEmpty(Rs!puesto) Then
    ''            Rs!puesto = ""
    ''        End If
    ''    End If
    ...
    '        Else
    '            Valores(0) = ""
    '            HOOK_Despues_de_modificar_campo_en_modificacion_ficha = True
    '        End If
    '        HOOK_AsignarValores(CT, Campos, Valores)
    ...
    Public Overrides Function HOOK_Despues_de_modificar_campo_en_modificacion_grid(CT As CnTabla.CnTabla, CE As CnEdicion.CnEdicion, Valor As String, GE As DataGridView, HH As Hashtable) As Boolean
        ...
        If CT.Estado = CnTabla.CnTabla.EstadoCnTabla.Modificando And Trim(CE.Campo) = "hora_marcaje" Then
            ReDim Preserve Campos(0), Valores(0)
            Campos(0) = "hora_marcaje" : Valores(0) = ""
            ...
            Else
                Valores(0) = ""
                HOOK_Despues_de_modificar_campo_en_modificacion_grid = True
            End If
            HOOK_AsignarValores(CT, Campos, Valores)

        ElseIf CT.Estado = CnTabla.CnTabla.EstadoCnTabla.Modificando And Trim(CE.Campo) = "tipo_marcaje" Then
            ReDim Campos(0), Valores(0)
            ...
            Else
                ...
                HOOK_Despues_de_modificar_campo_en_modificacion_grid = True
            End If
        ElseIf CT.Estado = CnTabla.CnTabla.EstadoCnTabla.Modificando And Trim(CE.Campo) = "actualizado" Then
            ReDim Campos(0), Valores(0)
    ...
    'Public Overridable Function HOOK_Antes_de_ModificarFila_en_modificacion(CT As CnTabla.CnTabla) As Boolean  'Comentado Probado Ejemplo
    '    HOOK_Antes_de_ModificarFila_en_modificacion = False
    'End Function

    Public Overrides Function HOOK_Antes_de_InsertarFila_en_creacion(CT As CnTabla.CnTabla) As Boolean 'Comentado Probado Ejemplo
        Dim Campos() As String, Valores() As String

        If CT.Estado = CnTabla.CnTabla.EstadoCnTabla.AceptandoCreacion And Trim(CT.Tabla) = "personal_marcajes" Then
            If String.IsNullOrEmpty(CT.RsCreacion("puesto")) And CT.RsCreacion("tipo_marcaje") = "S" Then
                ReDim Campos(0), Valores(0)
                Campos(0) = "puesto" : Valores(0) = ""
                HOOK_AsignarValores(CT, Campos, Valores)
            ElseIf String.IsNullOrEmpty(CT.RsCreacion("puesto")) And CT.RsCreacion("tipo_marcaje") = "E" Then
                MsgBox("Las entradas deben de tener un puesto")
                Return True
            End If
        End If
    ...
    Private Function VerificarHora(ByRef cHora As String) As Boolean
